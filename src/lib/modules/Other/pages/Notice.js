...
@inject(['otherNotice', 'global'])
@connect(({ otherNotice, global, loading }) => ({
  otherNotice,
  global,
  loading: loading.models.otherNotice,
  listLoading: loading.effects['otherNotice/noticeList'],
  typeLoading: loading.effects['otherNotice/noticeType'],
}))
export default class Notice extends PureComponent {
  ...
  submitForm = (form, fields) => {
    ...
    const params = fields;
    me.props.dispatch({
      type: 'otherNotice/submitOrUpdate',
      payload: {
        id: params.id,
  ...
  handleRemove = (record) => {
    const me = this;
    me.props.dispatch({
      type: 'otherNotice/removeNoticeInfo',
      payload: {
        id: record.id
      },
      callback: (res) => {
  ...
  filterTable = (inputValue, filter) => {
    const { otherNotice: { noticeList } } = this.props;
    this.setState({
      list: inputValue ? filter(noticeList, ['title', 'info', 'beginTime', 'endTime']) : noticeList
    })
  }
  render() {
    const { loading, global: { size }, listLoading, typeLoading, otherNotice: { menusType } } = this.props;
